! function(a, b) {
    ...
    function j(a, b) {
        if (b = b.trim(), "" !== b)
            if (-1 === b.indexOf(" ")) i(a, b) === !1 && (a.classList ? a.classList.add(b) : a.className += " " + b);
            else
                for (var c = b.replace(/\s+/g, " ").split(" "), d = 0, e = c.length; e > d; d++) j(a, c[d])
    }

    function k(a, b) {
        if (b = b.trim(), "" !== b)
            if (-1 === b.indexOf(" ")) i(a, b) && (a.classList ? a.classList.remove(b) : a.className = a.className.replace(new RegExp("(\\s|^)" + b + "(\\s|$)"), " "));
            else
                for (var c = b.replace(/\s+/g, " ").split(" "), d = 0, e = c.length; e > d; d++) k(a, c[d])
    }
